...
if st.button("Calculate"):
    ...
    expected_savings_by_now = calculate_savings(
        st.session_state['current_age'] - 1,
        st.session_state['current_age'],
        0,
        st.session_state['annual_savings'],
        st.session_state['annual_return']
    )
    # st.write(f"Progress towards target: {'ahead' if st.session_state['current_savings'] >= expected_savings_by_now else 'behind'} schedule")
